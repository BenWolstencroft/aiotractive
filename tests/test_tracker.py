...
async def test_tracker_details(auth_response, tracker_details):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/tracker/{TRACKER_ID}", payload=tracker_details)
...
async def test_tracker_hw_info(auth_response, hw_info):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/device_hw_report/{TRACKER_ID}/", payload=hw_info)
...
async def test_tracker_pos_report(auth_response, pos_report):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/device_pos_report/{TRACKER_ID}", payload=pos_report)
...
async def test_tracker_positions_history(auth_response, positions_history):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        pattern = re.compile(rf"^{re.escape(API_URL)}/tracker/{TRACKER_ID}/positions\?.*$")
        m.get(pattern, payload=positions_history)
...
async def test_tracker_buzzer_control(auth_response, active, action):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/tracker/{TRACKER_ID}/command/buzzer_control/{action}", payload={"pending": True})
...
async def test_tracker_led_control(auth_response):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/tracker/{TRACKER_ID}/command/led_control/on", payload={"pending": True})
...
async def test_tracker_live_tracking(auth_response):
    with aioresponses() as m:
        mock_auth(m, auth_response)
        m.get(f"{API_URL}/tracker/{TRACKER_ID}/command/live_tracking/on", payload={"pending": True})
...
async def test_tracker_repr(auth_response):
    with aioresponses() as m:
        mock_auth(m, auth_response)
