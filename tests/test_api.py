...
class TestAuthentication:
    ...
    async def test_success(self, aresponses: ResponsesMockServer, auth_response):
    ...
    async def test_caches_credentials(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)
    ...
    async def test_invalid_credentials(self, aresponses: ResponsesMockServer, status):
...
class TestRequestErrors:
    ...
    async def test_404_raises_not_found(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)
    ...
    async def test_500_raises_tractive_error(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)
    ...
    async def test_429_raises_tractive_error(self, aresponses: ResponsesMockServer, auth_response):
        """
...
class TestUserIdAndHeaders:

    async def test_user_id(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)

        async with Tractive("test@example.com", "password") as client:
            assert await client._api.user_id() == "test_user_123"

    async def test_auth_headers(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)
