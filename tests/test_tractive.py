...
class TestTractiveClient:

    async def test_context_manager_closes_session(self, aresponses: ResponsesMockServer, auth_response):
    ...
    async def test_trackers_list(self, aresponses: ResponsesMockServer, auth_response, tracker_data):
        mock_auth(aresponses, auth_response)
        aresponses.add(
            "graph.tractive.com",
    ...
    async def test_trackable_objects_list(self, aresponses: ResponsesMockServer, auth_response, trackable_object_data):
        mock_auth(aresponses, auth_response)
        aresponses.add(
            "graph.tractive.com",
    ...
    async def test_tracker_by_id_no_network_call(self, aresponses: ResponsesMockServer, auth_response):
    ...
    async def test_trackable_object_by_id_no_network_call(self, aresponses: ResponsesMockServer, auth_response):
        mock_auth(aresponses, auth_response)
...
class TestExternalSession:
    ...
    async def test_external_session_not_closed_by_client(self, aresponses: ResponsesMockServer, auth_response):
